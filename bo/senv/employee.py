...
仿真虚空间 Employee = 系统空间 Employee 的完整结构复制 +
扩展 task_id(仿真任务ID) / batch_no(仿真任务批次号) 两个属性，
用于唯一标识当前人员对象所属的仿真任务/批次。
"""
...
class Employee(BaseModel):
    """仿真虚空间 · Employee 持久化业务对象 (对应表 senv_employee)。"""
    ...
    emp_code: str = Field(min_length=1, max_length=64, description="人员编码(任务批次内唯一)")
    ...
    org_id: Optional[int] = Field(default=None, description="归属组织ID(对应 senv_organization.id)")
    ...
    @field_validator("emp_code", "emp_name", "task_id", "batch_no")
    @classmethod
